...
<div class="container">
  <div class="mt-3">
    <form class="row" method="post" action="/student?action=search">
      <div class="col-6">
  ...
  <table class="table table-hover">
    ...
    <c:forEach var="student" items="${students}" varStatus="status">
      <tr>
        ...
        <td>${student.point}</td>
        <td>
<%--            &lt;%&ndash;                    <c:if test="${student.point >= 8}">Học sinh giỏi</c:if>&ndash;%&gt;--%>
<%--            &lt;%&ndash;                    <c:if test="${student.point >= 6.5 && student.point < 8}">Học sinh khá</c:if>&ndash;%&gt;--%>
<%--            &lt;%&ndash;                    <c:if test="${student.point >= 5 && student.point <6.5}">Học sinh trung bình</c:if>&ndash;%&gt;--%>
<%--            &lt;%&ndash;                    <c:if test="${student.point >= 3.5 && student.point < 5}">Học sinh yếu</c:if>&ndash;%&gt;--%>
<%--            &lt;%&ndash;                    <c:if test="${student.point < 3.5}">Cook</c:if>&ndash;%&gt;--%>
          <c:choose>
        ...
        <td>
          ...
          <button class="btn b tn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal${student.id}">Xóa</button>
